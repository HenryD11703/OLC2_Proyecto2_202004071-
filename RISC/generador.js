...
export class Generator {
    ...
    la(rd, label) { // cargar una etiqueta en un registro
        this.instructions.push(new Instruction('la', rd, label));
    }
    ...
    printChar() { // imprimir un caracter en pantalla
        this.li(reg.A7, 11);
        this.ecall();
    }
    ...
    */
    ...
    toString() {
        this.comment('Generated by RISC-V Generator');
        this.endProgram();
        this.comment('Built-in functions');

        this.instruccionesFuncion.forEach(instruccion => {
            this.instructions.push(instruccion);
        });

        Array.from(this._builtInFunctions).forEach(name => {
            this.addLabel(name);
            builtfuncs[name](this);
            this.ret();
        });
